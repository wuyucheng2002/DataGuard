...
class Data:
    def __init__(self, target='aac084', u='aac009') -> None:  # 离退休状态作为target属性，户口性质作为隐私属性
        ...
        self.train_dataset_target, self.test_dataset_target = train_test_split(self.dataset_target, test_size=0.2,
                                                                               random_state=42)
        self.train_dataset_extractor, self.test_dataset_extractor = train_test_split(self.dataset_extractor,
        ...
        self.train_features = self.train_dataset_extractor[:, :-1]
        self.train_label = self.train_dataset_extractor[:, -1]

        self.test_features = self.test_dataset_extractor[:, :-1]
        self.test_label = self.test_dataset_extractor[:, -1]
        ...
        self.num_of_features = self.train_dataset_extractor[:, :-1].shape[1]  # extractor任务特征个数
        self.num_of_label_target = len(self.target.value_counts())  # target任务标签个数
        self.num_of_label = len(self.u.value_counts())
        ...
        if "Dataset" == "Adult_new":
            ...
            self.num_of_label = len(self.train_dataset[self.u].value_counts())  # extractor任务标签个数(敏感属性取值个数)

            # target任务
            self.train_features_target = self.train_dataset_target[:, :-1]  # 训练集X
            self.train_label_target = self.train_dataset_target[:, -1]  # 训练集y
            self.test_features = self.test_dataset_target[:, :-1]  # 测试集X
            self.test_label = self.test_dataset_target[:, -1]  # 测试集y

            # extractor任务
            self.train_features = self.train_dataset_extractor[:, :-1]  # 训练集X
            self.train_label = self.train_dataset_extractor[:, -1]  # 训练集y
    ...
    #     """
    #
    #     X_train_raw = train_dataset.drop(columns=['label'])
    #     y_train = np.array(train_dataset['label'])
    #
    #     X_test_raw = test_dataset.drop(columns=['label'])
...
if __name__ == '__main__':
    ...
    print(data.num_of_features)
    print(data.num_of_label)
