...
from my_TIPRDC.train_extractor import train, test_classifier, train_Z, train_classifier
...
def get_FE(data, epochs=50, batch_size=256, lr=0.1, lbd=0.66):
    ...
    """
    weights_FE = np.load('./model/pre_train/FE.npz', allow_pickle=True)['w']
    weights_CF = np.load('./model/pre_train/CF.npz', allow_pickle=True)['w']
    ...
    np.savez('./model/all_component/FE.npz', w=weights_FE)
    np.savez('./model/all_component/CF.npz', w=weights_CF)
    np.savez('./model/all_component/MI.npz', w=weights_MI)
...
def get_ZFE(data, epochs=50, batch_size=256, lr=0.1):
    ...
    weights_FE = FE.get_weights()
    weights_CF = CF.get_weights()
    np.savez('./model/FE_and_CF/FE.npz', w=weights_FE)
    np.savez('./model/FE_and_CF/CF.npz', w=weights_CF)
...
def get_classifier(FE_path, data, epochs=50, batch_size=256, lr=0.1):
    ...
    weights_CF = CF.get_weights()
    np.savez('./model/only_CF/CF.npz', w=weights_CF)
